document.addEventListener("DOMContentLoaded", function() {
    ...
    // FullCalendar セクション
    if (document.getElementById("hunter-calendar")) {
        ...
        const calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            events: hunterLogs.map(log => ({
                title: `${log.animal?.name ?? '不明'} ${log.count ?? 1}頭`,
                start: log.capture_date,
                extendedProps: {
                    prefecture: log.prefecture
                }
            })),
            eventDidMount: function(info) {
                // 猟獲日ごとの背景色やスタイルのカスタマイズができる場所
                info.el.style.backgroundColor = '#d4edda'; // 薄緑
            }
        });
    ...
    // 各ボタンのハンドリング、Mapbox、カレンダー初期化をここに集約
    initButtonHandlers();
    initAnimalSelection();
    initCountSelection();
    initWeatherSelection();
    initHuntingMethodSelection();
    initMapboxLogMap();
    initMapboxDashboardMap();
    initFullCalendar();
});
